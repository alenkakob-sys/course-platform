...
- **Форма в адмінці для створення/редагування курсів і уроків.** Зараз
  наповнення (курси, уроки, відео, презентації) додається напряму через
  Table Editor у Supabase — це вже без коду, але не такий зручний
  інтерфейс, як окрема форма на сайті. Можу зробити наступним кроком.
...
4. **Після деплою сайту** (крок 4) зареєструйте webhook одним запитом:
   ```bash
   curl "https://api.telegram.org/bot<ТОКЕН>/setWebhook" \
     -d "url=https://ваш-сайт.netlify.app/api/telegram/webhook" \
